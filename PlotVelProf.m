...
subplot = @(m,n,p) subtightplot (m, n, p, [0.04 0.03], ...
                                          [0.08 0.02], [0.065 0.01]);
...
for t = time
    ...
    for p = 1:3
        ...
        for i = 1:length(xp)
            ...
            ylabel('z [m]')
            text(xs+(xp(1)-xs)*1/8-0.04*diff*1e-6,10,num2str(0),'fontsize',7)
            text(xs+(xp(1)-xs)*1/8-0.1*diff*1e-6 + diff*1e-6,10,num2str(diff*1e-6/pm(p)),'fontsize',7)
            text(xs+(xp(1)-xs)*1/8-0.06*diff*1e-6 + diff/2*1e-6,10,num2str(diff/2*1e-6/pm(p)),'fontsize',7)
            if p == 1
                % Draw the velocity profiles
                plot(Up(i,is(3):ie(3)-1)*pm(p)+xp(i),zz,'-k')  
                set(gca,'XTickLabel','');
                text(xs+(xp(1)-xs)*1/8-diff*0.4*1e-6 + diff/2*1e-6,...
    ...
    set(gcf,'PaperUnits','centimeters','PaperPosition',[0 0 14 10.5],...
        'PaperPositionMode','manual');
    print('-r600','-depsc',['../Paper/Vel_Prof_' plottype '_' ...
                             num2str(t/60,3) 'min.eps']);
end
